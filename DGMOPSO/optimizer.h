...
public:
	...
	friend void optimize();
	double constrViolation(double constrValue, int constraIndex);
	bool evaluateParticles(const int& iternumber, const bool& useOpenMP, const bool& useMPI, const bool& useOpenCL, int swarmsize, double **swarm, double **Jswarm, double **Cswarm);
	...
	void setInitialization(int init, char* file_name = 0, int index = 0){m_initialization = init; m_filenameInit = file_name; m_indexInit = index; }
	...
	void setMultiThreadType(int type){ m_multiThreadType=type; }
	...
	int getInitialization(){return m_initialization;}
...
protected:
	...
	int m_initialization;		// = 0: random initialization, = 1: initialization from a given solution, = 2: initialization from a given file
	char* m_filenameInit;
	int m_indexInit;
	int m_nIterReverseComm;
	bool m_feasibleOnly;
	bool m_multiThread;			//no: serial execution, yes: parallel execution
	int m_multiThreadType;		//only for multiThread=yes --> 1: OpenMP, 2: MPI, 3: OpenCL
};
